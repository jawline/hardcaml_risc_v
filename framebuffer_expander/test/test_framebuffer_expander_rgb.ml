...
let%expect_test "test" =
  let pixel r g b =
    concat_lsb
      [ of_int_trunc ~width:8 r; of_int_trunc ~width:8 g; of_int_trunc ~width:8 b ]
  in
  ...
  let default_row = Bits.concat_lsb [ empty_half_row; full_half_row ] in
  let other_row = Bits.concat_lsb [ full_half_row; empty_half_row ] in
  let framebuffer_0 =
    List.init ~f:(fun i -> if i % 2 = 0 then default_row else other_row) 32
    |> Bits.concat_lsb
    |> Bits.split_lsb ~part_width:8
  in
  test ~framebuffers:[| framebuffer_0 |];
  [%expect
    {|
    (testing (length 6144))
    Framebuffer 0
