...
let%expect_test "test" =
  let strided =
    List.init ~f:(fun i -> i % 2 = 0) 32 |> List.map ~f:Bits.of_bool |> Bits.concat_msb
  in
  let rev_strided =
    List.init ~f:(fun i -> i % 2 = 1) 32 |> List.map ~f:Bits.of_bool |> Bits.concat_msb
  in
  let framebuffer_alternating_rows =
    List.init ~f:(fun _ -> [| strided; rev_strided |]) (32 / 2) |> Array.concat
  in
  let framebuffer_alternating_rows_2 =
    List.init ~f:(fun _ -> [| rev_strided; Bits.of_int ~width:32 0 |]) (32 / 2)
    |> Array.concat
  in
  let framebuffer_alternating_rows_3 =
    List.init ~f:(fun _ -> [| rev_strided; strided |]) (32 / 2) |> Array.concat
  in
  test
    ~name:"/tmp/test_framebuffer_expander"
    ~framebuffers:
      [| framebuffer_alternating_rows
       ; framebuffer_alternating_rows_2
       ; framebuffer_alternating_rows_3
      |];
  [%expect
    {|
    --**--**--**--**--**--**--**--**--**--**--**--**--**--**--**--**
