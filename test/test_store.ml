...
module Test_machine = struct
  ...
  let create (scope : Scope.t) ({ I.clock; clear; enable; funct3; destination ; value } : _ I.t) =
...
let print_ram sim = 
  ...
  Array.map
    ~f:(fun  mut -> Bits.Mutable.to_bits mut |> Bits.to_int)
    ram |> Array.iter ~f:(fun v -> printf "%02x " v);
    printf "\n";;
...
let%expect_test "lw" =
  ...
  Array.iter
    ~f:(fun  mut -> Bits.Mutable.copy_bits ~src:(Bits.of_int ~width:32 0xFF) ~dst:mut)
    initial_ram;
  let waveform, sim = Waveform.create sim in
  (* Aligned store, we expect these to succeed. *)
  (try test ~destination:0 ~value:0xAA ~funct3:(Funct3.Store.to_int Funct3.Store.Sw) sim with
  ...
  [%expect
    ...
    │                  ││────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────│
    │                  ││────────┬───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────│
    │store$current_stat││ 0      │3                                                                                                                      │
    │                  ││────────┴───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────│
    │store$funct3_is_er││                                                                                                                                │
    └──────────────────┘└────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
    04657d17c6e5758786e06cd152cdc491 |}]
;;
