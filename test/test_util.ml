...
let program_ram sim memory_parts =
  ...
  let bits_no_padding = Array.to_list memory_parts |> Bits.concat_lsb in
  let bits = Bits.(concat_lsb [ bits_no_padding; zero 32 ]) in
  let bits = Bits.split_lsb ~part_width:8 bits in
