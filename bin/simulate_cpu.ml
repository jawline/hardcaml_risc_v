...
let restore_mode fd original_mode =
  tcsetattr original_mode fd ~mode:TCSAFLUSH 

let with_raw_mode ~f =
  let fd = Core_unix.descr_of_out_channel Out_channel.stdout in

  if not (isatty fd) then (
          raise_s [%message "Cannot enter raw"]
  ) else
...
let clear_screen () =
  Out_channel.output_string Out_channel.stdout "\x1b[2J"
;;
  ...
  Command.basic
    ...
    (let open Command.Let_syntax in
     ...
     fun () ->
             ...
             test 
             ~before_printing_frame:(fun () -> clear_screen (); move_cursor_home () )
             ~print_frames:true ~cycles:100_000_000_000_000 ~data:program sim )
       )
;;
