...
let test ~verbose ~name ~clock_frequency ~baud_rate ~include_parity_bit ~stop_bits ~address ~packet
  =
  let all_inputs =
  ...
  let issue_read ~address ~length =
    ...
    if String.(without_length  <> packet) then raise_s [%message "BUG: Packet differs" ~received:(without_length) ~packet:(packet)];
    if verbose then (
